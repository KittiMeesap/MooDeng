...
{
    private const string apiKey = "AIzaSyA-hXmVy53-wA7J7ypXSlo5zW0k1EMt7EM"; // You can find this in your Firebase project settings
    ...
    /// <param name="password"> User's password </param>
    /// 
    public static void SignIn(string email, string password, Action<User> onSuccess, Action onFailure)
    {
        var payLoad = $"{{\"email\":\"{email}\",\"password\":\"{password}\",\"returnSecureToken\":true}}";
        RestClient.Post($"https://www.googleapis.com/identitytoolkit/v3/relyingparty/verifyPassword?key={apiKey}",
            ...
            response =>
            {
                ...
                CheckEmailVerification(authResponse["idToken"], () =>
                {
                    ...
                    DatabaseHandler.GetUser(authResponse["localId"], user =>
                    {
                        Debug.Log($"{user.name}");
                        onSuccess(user); // ส่ง user กลับไปยัง Register.cs ผ่าน callback
